/* ============================================================
   bg3d.js — 5 rotating Three.js 3D backgrounds
   Auto-switches every 2 minutes. Manual via dots / switchBG().
   ============================================================ */
...
(function () {
  ...
  (function () {
    ...
    for (let i = 0; i < 1500; i++) {
      ...
      ap.push(
        Math.cos(a) * r, (Math.random() - 0.5) * 1.5, Math.sin(a) * r,
        Math.cos(a + Math.PI) * r, (Math.random() - 0.5) * 1.5, Math.sin(a + Math.PI) * r
      );
    }
    const ag = new THREE.BufferGeometry();
    ag.setAttribute('position', new THREE.BufferAttribute(new Float32Array(ap), 3));
    scene.add(new THREE.Points(ag, new THREE.PointsMaterial({
      size: 0.08, color: 0x00f5ff, transparent: true, opacity: 0.35
    })));
  ...
  (function () {
    ...
    [
      ...
      const m = new THREE.Mesh(g, new THREE.MeshBasicMaterial({
        color: c, wireframe: true, transparent: true, opacity: 0.07
      }));
      m.position.set(...p);
      scene.add(m);
      objs.push(m);
    ...
    const N = 1500, p2 = new Float32Array(N * 3);
    for (let i = 0; i < N; i++) {
      p2[i * 3]     = (Math.random() - 0.5) * 100;
      p2[i * 3 + 1] = (Math.random() - 0.5) * 80;
      p2[i * 3 + 2] = (Math.random() - 0.5) * 40 - 20;
    }
    const pg = new THREE.BufferGeometry();
    pg.setAttribute('position', new THREE.BufferAttribute(p2, 3));
    scene.add(new THREE.Points(pg, new THREE.PointsMaterial({
      size: 0.1, color: 0x9b5de5, transparent: true, opacity: 0.4
    })));
      ...
      for (let y = -G; y <= G; y += 2) {
        ...
        const m = new THREE.Mesh(
          ...
          new THREE.MeshBasicMaterial({
            color: col,
            wireframe: col === 0x050508,
            transparent: true,
            opacity: col === 0x050508 ? 0.02 : 0.15
          })
        );
  ...
  (function () {
    ...
    [[0x00f5ff, 1], [0x9b5de5, -1], [0xf72585, 0.5]].forEach(([col, dir]) => {
      ...
      const pts = new THREE.Points(g, new THREE.PointsMaterial({
        size: 0.18, color: col, transparent: true, opacity: 0.5
      }));
      scene.add(pts);
